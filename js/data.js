...
// ── LOAD BALANCING ──
{id:3901,cat:"admin",titre:"Load Balancing — HAProxy",sub:"Frontend/backend, algorithmes, ACL, SSL termination, health checks",
 is_cmd:true,
 ...
 def:"HAProxy est un load balancer et proxy TCP/HTTP open-source haute performance : il répartit le trafic entre plusieurs serveurs backend, surveille leur santé (health checks) et permet un routage applicatif avancé via des ACL.",
 cmds:[
   {section:"Architecture & modes", items:[
     {cmd:"frontend web_front\n  bind *:80\n  mode http\n  default_backend web_servers", comment:"# Frontend = point d'écoute des clients. Mode HTTP (L7) : accès aux headers/cookies/URL, nécessaire pour les ACL applicatives"},
     {cmd:"backend web_servers\n  mode http\n  balance roundrobin", comment:"# Backend = groupe de serveurs cibles. Mode TCP (L4) = transparent mais aveugle au contenu (pas d'ACL sur URL/cookies)"}
   ]},
   {section:"Configuration de base — SSL et health check", items:[
     {cmd:"frontend web_front\n  bind *:80\n  bind *:443 ssl crt /etc/ssl/monsite.pem\n  redirect scheme https if !{ ssl_fc }\n  default_backend web_servers", comment:"# SSL termination : HAProxy déchiffre le TLS et envoie en HTTP aux backends (réduit leur charge)"},
     {cmd:"backend web_servers\n  balance roundrobin\n  option httpchk GET /health\n  server web1 10.0.0.1:80 check inter 2s rise 2 fall 3\n  server web2 10.0.0.2:80 check inter 2s rise 2 fall 3\n  server web3 10.0.0.3:80 check inter 2s rise 2 fall 3 backup", comment:"# Health check + serveur de secours (backup), retiré/réintégré automatiquement du pool"}
   ]},
   ...
   {section:"ACL, stats et diagnostics", items:[
     {cmd:"acl is_api path_beg /api/", comment:"# ACL : chemin commence par /api/ — routage applicatif conditionnel (path_beg, hdr, src, method…)"},
     {cmd:"use_backend api_servers if is_api", comment:"# Rediriger l'API vers un backend dédié"},
     {cmd:"frontend stats\n  bind *:8080\n  stats uri /haproxy?stats", comment:"# Tableau de bord temps réel (connexions, santé des serveurs, débit)"},
     {cmd:"haproxy -c -f /etc/haproxy/haproxy.cfg", comment:"# Tester la configuration avant rechargement"},
     {cmd:"echo 'show stat' | socat stdio /var/run/haproxy.sock", comment:"# Stats en CLI en temps réel"}
   ]}
 ],
 piege:"balance source (sticky par IP) ne fonctionne pas derrière un NAT — tous les utilisateurs ont la même IP source et atterrissent sur le même serveur, annulant l'effet du load balancing. Autre piège : utiliser le mode TCP pour du trafic HTTP empêche toute ACL basée sur l'URL ou les cookies.",
 retenir:"Frontend = écoute clients, backend = serveurs cibles. roundrobin/leastconn/source/uri = algorithmes, cookie = sticky fiable. Mode HTTP (L7) nécessaire pour les ACL sur URL/cookies, mode TCP (L4) = transparent mais aveugle. SSL termination = HAProxy déchiffre, backends reçoivent du HTTP. Stats page pour le monitoring temps réel.",
 keywords:["HAProxy","load balancing","roundrobin","leastconn","balance source","sticky session","health check","ACL","backend","frontend","SSL termination","mode HTTP","mode TCP","stats","haute disponibilité"]},
...
// WEF / LOGS (complément)
// ────────────────────────────────────────────────────────
{id:4927,cat:"wef",titre:"Syslog — Protocole et centralisation des logs",sub:"RFC 5424, PRI = Facility×8+Severity, TCP/TLS, rsyslog, journald, SIEM",
 ...
 def:"Syslog est le protocole standard de collecte et de centralisation des journaux d'événements sur les systèmes et équipements réseau — comprendre sa structure et son déploiement sécurisé est essentiel pour la supervision et la forensique.",
 points:[
   "Format RFC 5424 : <PRI>VERSION TIMESTAMP HOSTNAME APP-NAME PROCID MSGID STRUCTURED-DATA MSG — remplace RFC 3164 (2009 : format structuré, UTF-8, timezone garantis)",
   "PRI = (Facility × 8) + Severity. Ex: auth(4) + err(3) = PRI 35. Facilities principales : 0=kern, 1=user, 4=auth, 16-23=local0 à local7 (équipements réseau Cisco)",
   "UDP 514 = rapide mais sans garantie de livraison, non chiffré (perte silencieuse possible). TCP 6514 avec TLS (RFC 5425) = recommandé en production",
   "rsyslog : démon syslog Linux le plus répandu, client ET serveur. Fichier /etc/rsyslog.conf, rechargement via systemctl restart rsyslog. imudp/imtcp = entrée réseau, omfwd = forwarding",
   "journald (systemd) : stocke les logs en binaire localement — conversion vers syslog classique via le module imjournal de rsyslog",
   "Centraliser immédiatement vers un serveur distant (SIEM : Wazuh, Splunk, ELK, Graylog) limite le risque de tampering — un attaquant qui compromet une machine ne peut plus effacer les traces déjà envoyées"
 ],
 piege:"Envoyer les logs en UDP 514 sans TLS expose leur contenu en clair ET risque une perte silencieuse (paquets non garantis, notamment en forensique si le réseau était saturé). En production, toujours préférer TCP 6514 avec TLS.",
 retenir:"PRI = Facility×8 + Severity, 0=critique → 7=debug. RFC 5424 (structuré) remplace RFC 3164. UDP 514 = simple mais non fiable/non chiffré, TCP 6514+TLS = recommandé en prod. rsyslog = démon Linux (client+serveur), journald = stockage local systemd. Centraliser vite = anti-tampering.",
 keywords:["syslog","RFC 5424","rsyslog","syslog-ng","journald","facility","severity","UDP 514","TCP 6514","TLS","centralisation","tampering","SIEM","Wazuh","Graylog"]},
...
,{id:5109,cat:"secu",titre:"Analyse mémoire — Volatility",sub:"Dump RAM, pslist, malfind, netscan",
 schema:`<svg viewBox="0 0 440 130" xmlns="http://www.w3.org/2000/svg"><defs><marker id="mf-a" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z" class="sd-arrowhead"/></marker></defs><text class="sd-label" x="220" y="12">Analyse mémoire — du dump RAM aux artefacts</text><rect class="sd-box" x="6" y="30" width="90" height="40" rx="4"/><text class="sd-text-small" x="51" y="46">RAM live</text><text class="sd-text-small" x="51" y="58">(machine cible)</text><line class="sd-arrow" x1="96" y1="50" x2="130" y2="50" marker-end="url(#mf-a)"/><rect class="sd-box" x="132" y="30" width="90" height="40" rx="4"/><text class="sd-text-small" x="177" y="46">Dump RAM</text><text class="sd-text-small" x="177" y="58">(FTK/DumpIt)</text><line class="sd-arrow" x1="222" y1="50" x2="256" y2="50" marker-end="url(#mf-a)"/><rect class="sd-box-accent" x="258" y="30" width="90" height="40" rx="4"/><text class="sd-text-small" x="303" y="50">Volatility</text><line class="sd-arrow" x1="348" y1="50" x2="382" y2="50" marker-end="url(#mf-a)"/><rect class="sd-box" x="384" y="14" width="50" height="24" rx="3"/><text class="sd-text-small" x="409" y="28">pslist</text><rect class="sd-box" x="384" y="42" width="50" height="24" rx="3"/><text class="sd-text-small" x="409" y="56">malfind</text><rect class="sd-box" x="384" y="70" width="50" height="24" rx="3"/><text class="sd-text-small" x="409" y="84">netscan</text><text class="sd-label" x="220" y="104">Capture un instant T : révèle le fileless, invisible sur disque</text><text class="sd-label" x="220" y="120">malfind = détecte les injections de code en mémoire</text></svg>`,
 def:"L'analyse mémoire (memory forensics) consiste à examiner un dump RAM pour retrouver des processus, connexions réseau ou artefacts qu'une investigation disque seule ne verrait pas.",
 is_cmd:true,
 cmds:[
   {section:"Identifier le profil / OS", items:[
     {cmd:"vol.py -f memdump.raw imageinfo", comment:"# Volatility 2 : détecter le profil du système (OS/version)"},
     {cmd:"vol -f memdump.raw windows.info", comment:"# Volatility 3 : infos système équivalentes"}
   ]},
   {section:"Explorer les processus", items:[
     {cmd:"vol -f memdump.raw windows.pslist", comment:"# Lister les processus actifs au moment du dump"},
     {cmd:"vol -f memdump.raw windows.pstree", comment:"# Arborescence parent/enfant — repère les process suspects"},
     {cmd:"vol -f memdump.raw windows.malfind", comment:"# Détecter les injections de code en mémoire (process hollowing…)"}
   ]},
   {section:"Réseau et artefacts", items:[
     {cmd:"vol -f memdump.raw windows.netscan", comment:"# Connexions réseau actives/récentes au moment du dump"},
     {cmd:"vol -f memdump.raw windows.cmdline", comment:"# Ligne de commande utilisée pour lancer chaque processus"},
     {cmd:"vol -f memdump.raw windows.dumpfiles --pid 1234", comment:"# Extraire un fichier mappé en mémoire par un processus"}
   ]}
 ],
 piege:"Un dump mémoire capture un instant T : un malware fileless qui s'exécute uniquement en RAM (jamais écrit sur disque) n'apparaîtra QUE dans cette analyse, pas dans une investigation disque classique.",
 retenir:"Memory forensics = examiner un dump RAM (Volatility). pslist/pstree = processus, malfind = injections, netscan = connexions réseau au moment du dump.",
 keywords:["Volatility","memory forensics","dump mémoire","pslist","malfind","netscan","fileless","process hollowing"]}

,{id:5110,cat:"secu",titre:"Imagerie disque & chaîne de custody",sub:"Copie bit-à-bit, write blocker, hash, chain of custody",
 schema:`<svg viewBox="0 0 440 130" xmlns="http://www.w3.org/2000/svg"><defs><marker id="im-a" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z" class="sd-arrowhead"/></marker></defs><text class="sd-label" x="220" y="12">Imagerie disque — préserver la preuve</text><rect class="sd-box" x="6" y="34" width="90" height="40" rx="4"/><text class="sd-text-small" x="51" y="50">Disque</text><text class="sd-text-small" x="51" y="62">original</text><line class="sd-arrow" x1="96" y1="54" x2="130" y2="54" marker-end="url(#im-a)"/><rect class="sd-box-accent" x="132" y="20" width="76" height="24" rx="3"/><text class="sd-text-small" x="170" y="34">Write blocker</text><rect class="sd-box" x="214" y="34" width="100" height="40" rx="4"/><text class="sd-text-small" x="264" y="50">Image bit-à-bit</text><text class="sd-text-small" x="264" y="62">hash A ⇄ hash B</text><line class="sd-arrow" x1="314" y1="54" x2="348" y2="54" marker-end="url(#im-a)"/><rect class="sd-box" x="350" y="34" width="84" height="40" rx="4"/><text class="sd-text-small" x="392" y="50">Copies de</text><text class="sd-text-small" x="392" y="62">travail</text><text class="sd-label" x="220" y="100">Hash identique avant/après = intégrité prouvée</text><text class="sd-label" x="220" y="116">Chaîne de custody : qui, quand, où, pourquoi — à chaque étape</text></svg>`,
 def:"L'imagerie disque consiste à créer une copie bit-à-bit exacte d'un support de stockage pour l'analyser sans jamais toucher à l'original, condition indispensable pour que les preuves restent recevables.",
 points:["Copie bit-à-bit (pas une simple copie de fichiers) : dd, FTK Imager, dc3dd — capture aussi l'espace non alloué (fichiers supprimés récupérables)","Bloqueur en écriture (write blocker) matériel ou logiciel : empêche toute modification accidentelle du disque original pendant la capture","Vérification d'intégrité : hash SHA-256 calculé avant ET après copie — un hash identique prouve que l'image est fidèle à l'original","Chaîne de custody (chain of custody) : registre traçant qui a manipulé la preuve, quand, où et pourquoi, du prélèvement jusqu'au tribunal si besoin","On travaille uniquement sur une copie de l'image (jamais l'original ni même l'image maîtresse) pour préserver la preuve source","Format d'image courant : E01 (EnCase, avec métadonnées et compression) ou RAW/dd (brut, universel)"],
 piege:"Analyser directement le disque original sans bloqueur en écriture peut modifier des timestamps ou écrire des fichiers temporaires — la preuve devient alors contestable, voire irrecevable en justice.",
 retenir:"Imagerie = copie bit-à-bit avec write blocker + hash avant/après. Chaîne de custody = traçabilité complète de la preuve. On travaille toujours sur une copie, jamais l'original.",
 keywords:["imagerie disque","write blocker","chaîne de custody","hash SHA-256","FTK Imager","dd","preuve numérique","E01"]}

,{id:5111,cat:"hacking",titre:"Reverse Engineering — Analyse statique de malware",sub:"Strings, PE header, hash, YARA, packers",
 schema:`<svg viewBox="0 0 440 150" xmlns="http://www.w3.org/2000/svg"><defs><marker id="as-a" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z" class="sd-arrowhead"/></marker></defs><text class="sd-label" x="220" y="12">Analyse statique — sans jamais exécuter le fichier</text><rect class="sd-box-accent" x="6" y="40" width="90" height="40" rx="4"/><text class="sd-text-small" x="51" y="56">Fichier</text><text class="sd-text-small" x="51" y="68">suspect</text><line class="sd-arrow" x1="96" y1="60" x2="130" y2="60" marker-end="url(#as-a)"/><rect class="sd-box" x="132" y="20" width="90" height="28" rx="3"/><text class="sd-text-small" x="177" y="38">Hash → VT</text><rect class="sd-box" x="132" y="52" width="90" height="28" rx="3"/><text class="sd-text-small" x="177" y="70">strings</text><rect class="sd-box" x="132" y="84" width="90" height="28" rx="3"/><text class="sd-text-small" x="177" y="102">PEstudio</text><line class="sd-arrow" x1="222" y1="60" x2="256" y2="60" marker-end="url(#as-a)"/><rect class="sd-box" x="258" y="40" width="90" height="40" rx="4"/><text class="sd-text-small" x="303" y="56">Packer ?</text><text class="sd-text-small" x="303" y="68">(UPX…)</text><line class="sd-arrow" x1="348" y1="60" x2="382" y2="60" marker-end="url(#as-a)"/><rect class="sd-box" x="384" y="40" width="50" height="40" rx="4"/><text class="sd-text-small" x="409" y="60">YARA</text><text class="sd-label" x="220" y="130">Rapide et sans risque — mais limité face au code obfusqué/packé</text></svg>`,
 def:"L'analyse statique examine un fichier suspect sans l'exécuter, en étudiant sa structure et son contenu pour identifier des indicateurs de malveillance.",
 points:["Hash et réputation : SHA-256 du fichier comparé à des bases connues (VirusTotal, MalwareBazaar) — identifie rapidement un malware déjà répertorié","Chaînes de caractères (strings) : strings malware.exe révèle parfois des URLs, clés de registre, messages d'erreur ou identifiants codés en dur","Analyse de l'en-tête PE (Windows) : PEview/PEstudio — sections, imports (API Windows utilisées), ressources embarquées, packers détectés","Packers et obfuscation : un exécutable packé (UPX…) cache son vrai code — nécessite un dépacking avant analyse approfondie","Signature YARA : règles de détection basées sur des motifs (chaînes, séquences d'octets, structure) pour classifier un échantillon","Analyse statique = zéro risque d'exécution accidentelle, mais limitée face au code fortement obfusqué ou chiffré"],
 piege:"Un malware bien obfusqué ou packé peut rendre l'analyse statique quasi inutile en apparence — c'est justement le signal qu'il faut basculer vers une analyse dynamique en environnement isolé.",
 retenir:"Analyse statique = examiner sans exécuter (hash, strings, en-tête PE, YARA). Rapide et sans risque, mais limitée face à l'obfuscation/packing.",
 keywords:["analyse statique","reverse engineering","strings","PEstudio","VirusTotal","YARA","packer","PE header","malware"]}

,{id:5112,cat:"hacking",titre:"Reverse Engineering — Analyse dynamique & sandboxing",sub:"Sandbox isolée, ProcMon, Wireshark, IoC",
 schema:`<svg viewBox="0 0 440 150" xmlns="http://www.w3.org/2000/svg"><text class="sd-label" x="220" y="12">Sandbox — isolation totale du réseau réel</text><rect class="sd-box" x="10" y="30" width="420" height="90" rx="6" style="stroke-dasharray:5,4"/><text class="sd-text-small" x="220" y="46">Environnement isolé (VM + snapshot)</text><rect class="sd-box-accent" x="30" y="56" width="90" height="40" rx="4"/><text class="sd-text-small" x="75" y="76">Malware</text><text class="sd-text-small" x="75" y="88">exécuté</text><rect class="sd-box" x="150" y="56" width="90" height="40" rx="4"/><text class="sd-text-small" x="195" y="72">ProcMon</text><text class="sd-text-small" x="195" y="86">(fichiers/registre)</text><rect class="sd-box" x="270" y="56" width="90" height="40" rx="4"/><text class="sd-text-small" x="315" y="72">Wireshark</text><text class="sd-text-small" x="315" y="86">(réseau/C2)</text><text class="sd-label" x="220" y="106">✕ Aucun accès au réseau de production pendant l'analyse</text><text class="sd-label" x="220" y="140">IoC extraits : hashes, IPs/domaines, clés de registre, mutex</text></svg>`,
 def:"L'analyse dynamique exécute le malware dans un environnement isolé (sandbox) pour observer son comportement réel : fichiers créés, connexions réseau, modifications système.",
 points:["Sandbox isolée : machine virtuelle sans accès au réseau de production, snapshot restauré après chaque analyse (Cuckoo Sandbox, ANY.RUN, Joe Sandbox)","Monitoring système : Process Monitor (ProcMon) trace les accès fichiers/registre en temps réel, Process Explorer visualise les processus créés","Capture réseau : Wireshark ou proxy (Fiddler) pour observer les connexions C2 (command & control), l'exfiltration, les domaines contactés","Indicateurs de compromission (IoC) extraits : hashes, IPs/domaines contactés, clés de registre créées, mutex, fichiers déposés","Techniques anti-analyse : certains malwares détectent l'environnement virtualisé (VM-aware) et modifient leur comportement pour tromper la sandbox","On travaille toujours dans un environnement totalement isolé du réseau réel — un malware actif peut se propager ou exfiltrer des données pendant l'analyse"],
 piege:"Exécuter un malware sur une machine connectée au réseau de l'entreprise, même juste pour voir, peut déclencher une propagation ou une exfiltration réelle — l'isolation n'est pas une précaution optionnelle.",
 retenir:"Analyse dynamique = exécuter en sandbox isolée et observer (ProcMon, Wireshark). Extrait des IoC concrets. Attention aux malwares VM-aware qui détectent la sandbox.",
 keywords:["analyse dynamique","sandbox","Cuckoo Sandbox","ProcMon","IoC","C2","command and control","VM-aware","isolation réseau"]}

,{id:5113,cat:"secu",titre:"YARA — Règles de détection de malware",sub:"Pattern matching, strings, condition, EDR",
 schema:`<svg viewBox="0 0 440 140" xmlns="http://www.w3.org/2000/svg"><defs><marker id="ya-a" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z" class="sd-arrowhead"/></marker></defs><text class="sd-label" x="220" y="12">Anatomie d'une règle YARA</text><rect class="sd-box" x="10" y="30" width="420" height="60" rx="4"/><text class="sd-text-small" x="90" y="48">rule nom_regle {</text><text class="sd-text-small" x="140" y="64">strings: $s1 = "texte" $s2 = {6A 40}</text><text class="sd-text-small" x="140" y="80">condition: $s1 or $s2 }</text><rect class="sd-box" x="10" y="100" width="200" height="30" rx="4"/><text class="sd-text-small" x="110" y="119">Fichier scanné (yara -r)</text><line class="sd-arrow" x1="210" y1="115" x2="260" y2="115" marker-end="url(#ya-a)"/><rect class="sd-box-accent" x="262" y="100" width="168" height="30" rx="4"/><text class="sd-text-small" x="346" y="119">Match si condition vraie</text></svg>`,
 def:"YARA est un outil de classification de fichiers par motifs (pattern matching) : une règle décrit des caractéristiques identifiant une famille de malware, très utilisée par les analystes et les EDR.",
 is_cmd:true,
 cmds:[
   {section:"Écrire une règle simple", items:[
     {cmd:"rule ransomware_exemple { strings: $s1 = \"YOUR FILES ARE ENCRYPTED\" $s2 = \"vssadmin delete shadows\" condition: any of them }", comment:"# Règle : match si l'une des deux chaînes est présente"}
   ]},
   {section:"Utiliser YARA en ligne de commande", items:[
     {cmd:"yara regle.yar fichier_suspect.exe", comment:"# Tester une règle contre un seul fichier"},
     {cmd:"yara -r regle.yar /chemin/dossier/", comment:"# Scanner récursivement un dossier"},
     {cmd:"yara -s regle.yar fichier_suspect.exe", comment:"# Afficher les chaînes qui ont matché (utile pour valider/débugger)"}
   ]}
 ],
 piege:"Une règle YARA trop générique (ex: une chaîne de 3 caractères) génère énormément de faux positifs — la condition doit combiner plusieurs indicateurs distinctifs, pas un seul motif isolé.",
 retenir:"YARA = pattern matching pour classifier des fichiers (strings + condition). Utilisé par les EDR et les analystes pour détecter des familles de malware connues.",
 keywords:["YARA","pattern matching","règle de détection","EDR","signature","strings","malware"]}

,{id:5114,cat:"crypto",titre:"Cryptographie post-quantique",sub:"ML-KEM, ML-DSA, Shor, hybride, harvest now decrypt later",
 schema:`<svg viewBox="0 0 440 150" xmlns="http://www.w3.org/2000/svg"><defs><marker id="pq-a" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z" class="sd-arrowhead"/></marker></defs><text class="sd-label" x="220" y="12">Échange de clé TLS — classique + post-quantique</text><rect class="sd-box" x="10" y="30" width="130" height="34" rx="4"/><text class="sd-text-small" x="75" y="50">ECDHE (classique)</text><rect class="sd-box" x="150" y="30" width="130" height="34" rx="4"/><text class="sd-text-small" x="215" y="50">ML-KEM (post-quantique)</text><rect class="sd-box-accent" x="290" y="22" width="140" height="50" rx="4"/><text class="sd-text-small" x="360" y="42">Secret combiné</text><text class="sd-text-small" x="360" y="58">(hybride)</text><line class="sd-arrow" x1="140" y1="47" x2="290" y2="42" marker-end="url(#pq-a)"/><line class="sd-arrow" x1="280" y1="47" x2="290" y2="52" marker-end="url(#pq-a)"/><text class="sd-label" x="220" y="100">Sécurisé même si un seul des deux échanges est cassé plus tard</text><text class="sd-label" x="220" y="120">Shor casse RSA/ECC · Grover ne fait que réduire AES-256 de moitié</text><text class="sd-label" x="220" y="138">« Harvest now, decrypt later » : intercepté aujourd'hui, déchiffré demain</text></svg>`,
 def:"La cryptographie post-quantique (PQC) désigne des algorithmes conçus pour résister à un ordinateur quantique suffisamment puissant, qui casserait RSA et ECC via l'algorithme de Shor.",
 points:["Menace « harvest now, decrypt later » : des données chiffrées aujourd'hui avec RSA/ECC peuvent être interceptées et stockées pour être déchiffrées plus tard, une fois l'ordinateur quantique disponible","Algorithme de Shor : sur un ordinateur quantique suffisamment puissant, il casse en temps raisonnable la factorisation (RSA) et le logarithme discret (ECC/ECDH) — pas encore réalisable en pratique en 2026, mais anticipé","NIST PQC : standardisation de nouveaux algorithmes résistants — ML-KEM (ex-Kyber) pour l'échange de clés, ML-DSA (ex-Dilithium) pour les signatures","Approche hybride : combiner un échange classique (ECDH) et un échange post-quantique (ML-KEM) dans la même session TLS — sécurisé même si l'un des deux est cassé un jour","AES (symétrique) reste globalement solide face au quantique : l'algorithme de Grover ne fait que réduire la sécurité effective de moitié (AES-256 reste largement suffisant)","Migration : chantier long (inventaire des usages de crypto, mise à jour des bibliothèques, tests de compatibilité) déjà engagé par les grands éditeurs (navigateurs, TLS)"],
 piege:"Le post-quantique protège contre une menace future, mais « harvest now, decrypt later » signifie que les données à confidentialité longue durée (secrets d'État, données médicales) sont déjà concernées aujourd'hui, pas seulement demain.",
 retenir:"PQC = résister à l'algorithme de Shor. ML-KEM (échange de clés) et ML-DSA (signatures) = nouveaux standards NIST. Approche hybride = classique + post-quantique en parallèle. AES reste solide (Grover, pas Shor).",
 keywords:["post-quantique","PQC","ML-KEM","Kyber","ML-DSA","Dilithium","algorithme de Shor","harvest now decrypt later","NIST","hybride"]}

,{id:5115,cat:"web",titre:"OAuth2 — Flows d'autorisation en détail",sub:"Authorization Code, PKCE, Client Credentials, OIDC",
 schema:`<svg viewBox="0 0 440 150" xmlns="http://www.w3.org/2000/svg"><defs><marker id="oa-a" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z" class="sd-arrowhead"/></marker></defs><text class="sd-label" x="220" y="12">Authorization Code + PKCE (simplifié)</text><rect class="sd-box" x="6" y="40" width="80" height="36" rx="4"/><text class="sd-text-small" x="46" y="61">Client</text><rect class="sd-box" x="180" y="10" width="90" height="36" rx="4"/><text class="sd-text-small" x="225" y="31">Serveur</text><text class="sd-text-small" x="225" y="43">d'autorisation</text><rect class="sd-box" x="354" y="40" width="80" height="36" rx="4"/><text class="sd-text-small" x="394" y="61">API cible</text><line class="sd-arrow" x1="86" y1="52" x2="176" y2="32" marker-end="url(#oa-a)"/><text class="sd-text-small" x="130" y="24">1. login + code_challenge</text><line class="sd-arrow" x1="176" y1="42" x2="86" y2="62" marker-end="url(#oa-a)"/><text class="sd-text-small" x="130" y="86">2. code d'autorisation</text><rect class="sd-box-accent" x="145" y="96" width="160" height="34" rx="4"/><text class="sd-text-small" x="225" y="110">3. échange code</text><text class="sd-text-small" x="225" y="122">+ code_verifier → token</text><line class="sd-arrow" x1="86" y1="70" x2="354" y2="70" marker-end="url(#oa-a)"/><text class="sd-label" x="220" y="146">4. Le client appelle l'API avec le token (Authorization: Bearer …)</text></svg>`,
 def:"OAuth2 définit plusieurs « flows » (grant types) selon le type de client — application web, mobile, ou machine-à-machine — pour obtenir un token d'accès de façon sécurisée.",
 points:["Authorization Code + PKCE : flow recommandé pour les apps web/mobiles — l'utilisateur s'authentifie sur le serveur d'autorisation, un code est échangé contre un token via un canal backend, PKCE empêche l'interception du code sur un client public","Client Credentials : pour la communication machine-à-machine (API-to-API), sans utilisateur humain — le client s'authentifie directement avec son client_id/client_secret","Device Code : pour les appareils sans navigateur pratique (TV connectée, IoT) — l'utilisateur valide l'accès depuis un autre appareil via un code affiché","Refresh token : permet d'obtenir un nouvel access token sans redemander les identifiants — access token de courte durée, refresh token plus long mais révocable","Scopes : limitent précisément ce que le token autorise (ex: read:profile, write:orders) — principe du moindre privilège appliqué à l'autorisation","OIDC (OpenID Connect) ajoute l'authentification par-dessus OAuth2 (qui ne fait que l'autorisation) via un id_token signé (JWT) contenant l'identité de l'utilisateur"],
 piege:"Le flow « Implicit » (token renvoyé directement dans l'URL) est obsolète et déconseillé depuis plusieurs années : le token s'expose dans l'historique du navigateur et les logs — toujours privilégier Authorization Code + PKCE.",
 retenir:"OAuth2 = autorisation (pas authentification, ça c'est OIDC). Authorization Code + PKCE = flow standard web/mobile. Client Credentials = machine-à-machine. Refresh token = renouveler sans ré-authentifier.",
 keywords:["OAuth2","PKCE","Authorization Code","Client Credentials","Device Code","refresh token","scopes","OIDC","OpenID Connect","access token"]}

,{id:5116,cat:"web",titre:"Sécurité GraphQL",sub:"Introspection, depth limiting, N+1, autorisation par champ",
 schema:`<svg viewBox="0 0 440 140" xmlns="http://www.w3.org/2000/svg"><text class="sd-label" x="220" y="12">Autorisation — REST (endpoint) vs GraphQL (champ par champ)</text><rect class="sd-box" x="10" y="30" width="200" height="30" rx="3"/><text class="sd-text-small" x="110" y="49">REST : /api/orders → 1 contrôle</text><rect class="sd-box-accent" x="230" y="26" width="200" height="80" rx="4"/><text class="sd-text-small" x="330" y="42">GraphQL : 1 requête</text><text class="sd-text-small" x="330" y="58">→ plusieurs resolvers</text><text class="sd-text-small" x="330" y="74">→ 1 contrôle par champ</text><text class="sd-text-small" x="330" y="90">(en oublier un seul = fuite)</text><text class="sd-label" x="220" y="122">Introspection + depth limiting + query cost = les 3 protections de base</text></svg>`,
 def:"GraphQL expose un langage de requête flexible où le client choisit précisément les données à récupérer, ce qui introduit des risques de sécurité différents de ceux d'une API REST classique.",
 points:["Introspection activée en production : révèle tout le schéma (types, champs, mutations disponibles) à un attaquant — à désactiver ou restreindre hors développement","Requêtes imbriquées profondes (nested queries) : une requête très imbriquée peut générer une explosion combinatoire de résultats côté serveur — nécessite une limite de profondeur (depth limiting)","Batching / attaques par lot : plusieurs requêtes envoyées en une seule requête HTTP peuvent contourner un rate limiting basé sur le nombre de requêtes HTTP","Problème N+1 : une requête mal résolue déclenche une requête base de données par élément d'une liste — impact performance ET porte ouverte à un déni de service (à limiter avec un data loader/batching interne)","Autorisation par champ (field-level) : contrairement à REST où l'endpoint filtre déjà l'accès, GraphQL nécessite de vérifier les droits à l'intérieur de chaque resolver, champ par champ","Query cost analysis : attribuer un coût à chaque champ/requête et rejeter celles dépassant un budget défini, en complément du depth limiting"],
 piege:"Contrairement à une API REST où chaque endpoint peut avoir son propre contrôle d'accès simple, oublier de vérifier les droits dans UN SEUL resolver GraphQL peut exposer un champ sensible à travers n'importe quelle requête qui l'inclut.",
 retenir:"Sécurité GraphQL = désactiver l'introspection en prod, limiter la profondeur des requêtes, autoriser champ par champ (pas juste au niveau endpoint), se méfier du batching et du problème N+1.",
 keywords:["GraphQL","introspection","depth limiting","N+1","batching","resolver","query cost","autorisation"]}

,{id:5117,cat:"methodo",titre:"Certifications cyber — Panorama et parcours",sub:"Security+, CEH, OSCP, ISO 27001, CISSP, CCNA",
 schema:`<svg viewBox="0 0 440 170" xmlns="http://www.w3.org/2000/svg"><text class="sd-label" x="220" y="12">Certifications cyber — repères par domaine</text><rect class="sd-box" x="10" y="26" width="130" height="30" rx="3"/><text class="sd-text-small" x="75" y="41">Généraliste</text><text class="sd-text-small" x="75" y="52">Security+ · CISSP</text><rect class="sd-box" x="150" y="26" width="130" height="30" rx="3"/><text class="sd-text-small" x="215" y="41">Pentest</text><text class="sd-text-small" x="215" y="52">CEH · OSCP</text><rect class="sd-box" x="290" y="26" width="140" height="30" rx="3"/><text class="sd-text-small" x="360" y="41">GRC / audit</text><text class="sd-text-small" x="360" y="52">ISO 27001 LA/LI</text><rect class="sd-box-accent" x="150" y="66" width="130" height="30" rx="3"/><text class="sd-text-small" x="215" y="81">Réseau / infra</text><text class="sd-text-small" x="215" y="92">CCNA</text><text class="sd-label" x="220" y="126">Débutant → Security+, CCNA, CEH (théorique)</text><text class="sd-label" x="220" y="144">Confirmé → OSCP (pratique), Lead Auditor, CISSP (5 ans XP)</text><text class="sd-label" x="220" y="160">Une certification ne remplace pas la pratique (labs, CTF, projets)</text></svg>`,
 def:"Les certifications professionnelles valident des compétences précises et sont souvent demandées par les recruteurs en cybersécurité, en complément d'un diplôme comme le BTS SIO.",
 points:["CompTIA Security+ : certification généraliste d'entrée, valide les bases transversales (réseau, crypto, gestion des risques, conformité) — bon premier palier après le BTS","CEH (Certified Ethical Hacker) : certification orientée pentest, couvre la méthodologie offensive et les outils — plutôt théorique/QCM, souvent un prérequis administratif pour certains postes","OSCP (Offensive Security Certified Professional) : certification pentest 100% pratique (examen de 24h sur des machines réelles à compromettre) — très reconnue sur le terrain, exigeante","ISO 27001 Lead Auditor / Lead Implementer : certifications GRC, valident la capacité à auditer ou déployer un SMSI selon la norme — utile pour évoluer vers RSSI ou consultant","CISSP (Certified Information Systems Security Professional) : certification généraliste de haut niveau, exige 5 ans d'expérience — vise plutôt un poste de management sécurité","CCNA : certification réseau Cisco, complémentaire (pas cyber pure) mais très valorisée pour les postes SISR/infra"],
 piege:"Une certification ne remplace pas l'expérience pratique : beaucoup de recruteurs valorisent davantage un projet perso (lab de pentest, CTF, contribution open source) qu'un titre seul sur un CV, surtout en début de carrière.",
 retenir:"Security+ = généraliste d'entrée. CEH/OSCP = pentest (OSCP = pratique, plus exigeant). ISO 27001 Lead Auditor = GRC. CISSP = management, 5 ans d'XP requis. CCNA = réseau/infra.",
 keywords:["Security+","CEH","OSCP","CISSP","ISO 27001","Lead Auditor","CCNA","certification","carrière cyber"]}

,{id:5118,cat:"methodo",titre:"Choisir sa certification après le BTS SIO",sub:"SOC, pentest, GRC, réseau : quel parcours pour quel objectif",
 schema:`<svg viewBox="0 0 440 170" xmlns="http://www.w3.org/2000/svg"><defs><marker id="cc-a" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z" class="sd-arrowhead"/></marker></defs><text class="sd-label" x="220" y="12">Quelle certification selon l'orientation ?</text><rect class="sd-box-accent" x="170" y="24" width="100" height="30" rx="4"/><text class="sd-text-small" x="220" y="43">BTS SIO</text><line class="sd-arrow" x1="195" y1="54" x2="60" y2="78" marker-end="url(#cc-a)"/><line class="sd-arrow" x1="210" y1="54" x2="165" y2="78" marker-end="url(#cc-a)"/><line class="sd-arrow" x1="230" y1="54" x2="270" y2="78" marker-end="url(#cc-a)"/><line class="sd-arrow" x1="250" y1="54" x2="377" y2="78" marker-end="url(#cc-a)"/><rect class="sd-box" x="10" y="80" width="95" height="40" rx="4"/><text class="sd-text-small" x="57" y="96">SOC / Blue</text><text class="sd-text-small" x="57" y="110">Security+, BTL1</text><rect class="sd-box" x="115" y="80" width="105" height="40" rx="4"/><text class="sd-text-small" x="167" y="96">Pentest / Red</text><text class="sd-text-small" x="167" y="110">eJPT → OSCP</text><rect class="sd-box" x="230" y="80" width="90" height="40" rx="4"/><text class="sd-text-small" x="275" y="96">GRC / Audit</text><text class="sd-text-small" x="275" y="110">ISO 27001 LI/LA</text><rect class="sd-box" x="330" y="80" width="100" height="40" rx="4"/><text class="sd-text-small" x="380" y="96">Réseau / Infra</text><text class="sd-text-small" x="380" y="110">CCNA</text><text class="sd-label" x="220" y="150">La pratique (CTF, labs, TryHackMe/HTB) compte autant que le papier</text></svg>`,
 def:"Le choix d'une certification dépend surtout de l'orientation professionnelle visée : défense (SOC/Blue Team), attaque (pentest/Red Team), gouvernance (GRC/audit) ou infrastructure réseau.",
 points:["Orientation SOC / Blue Team : privilégier Security+ puis Blue Team Level 1 (BTL1) ou GCIH — compétences en détection, analyse de logs, réponse à incident","Orientation pentest / Red Team : Security+ en base, puis eJPT (entrée pratique accessible) avant de viser OSCP à moyen terme — la pratique (labs, CTF) compte plus que les certifications à ce stade","Orientation GRC / audit / conformité : ISO 27001 Lead Implementer d'abord (déployer), puis Lead Auditor (auditer) — utile aussi de maîtriser EBIOS RM (déjà couvert dans les fiches dédiées)","Orientation infrastructure / réseau : CCNA reste la référence pour valider des bases réseau solides, complémentaire à une orientation cyber prise plus tard","Avant de payer une certification coûteuse, tester gratuitement l'équivalent : TryHackMe/HackTheBox pour le pentest, labs Cisco Packet Tracer pour le réseau, plateforme EBIOS RM de l'ANSSI pour la GRC","Un CTF (Capture The Flag) ou un lab personnel documenté sur GitHub est souvent un meilleur argument d'entretien qu'une certification seule, surtout sans expérience professionnelle"],
 piege:"Se lancer directement dans une certification très exigeante (OSCP, CISSP) sans bases solides ni pratique préalable mène souvent à l'échec ou à un apprentissage superficiel — mieux vaut progresser par palier.",
 retenir:"Le choix dépend du métier visé : SOC → Security+/BTL1, pentest → eJPT puis OSCP, GRC → ISO 27001, infra → CCNA. La pratique (CTF, labs) compte autant que le papier.",
 keywords:["orientation carrière","SOC","Red Team","Blue Team","GRC","CCNA","CTF","TryHackMe","HackTheBox","eJPT"]}

,{id:5119,cat:"ad",titre:"AD CS — Attaques sur les services de certificats",sub:"ESC1, ESC4, ESC8, Certipy, relais NTLM",
 schema:`<svg viewBox="0 0 440 170" xmlns="http://www.w3.org/2000/svg"><defs><marker id="adcs-a" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z" class="sd-arrowhead"/></marker></defs><text class="sd-label" x="220" y="12">ESC1 — SAN arbitraire dans une demande de certificat</text><rect class="sd-box" x="6" y="34" width="100" height="40" rx="4"/><text class="sd-text-small" x="56" y="50">Utilisateur</text><text class="sd-text-small" x="56" y="62">standard</text><line class="sd-arrow" x1="106" y1="54" x2="165" y2="54" marker-end="url(#adcs-a)"/><text class="sd-text-small" x="135" y="46">SAN=admin@dom</text><rect class="sd-box-accent" x="167" y="34" width="105" height="40" rx="4"/><text class="sd-text-small" x="219" y="50">AD CS</text><text class="sd-text-small" x="219" y="62">(template vulnérable)</text><line class="sd-arrow" x1="272" y1="54" x2="330" y2="54" marker-end="url(#adcs-a)"/><rect class="sd-box" x="332" y="34" width="100" height="40" rx="4"/><text class="sd-text-small" x="382" y="50">Certificat</text><text class="sd-text-small" x="382" y="62">"admin"</text><rect class="sd-box" x="6" y="96" width="426" height="60" rx="4"/><text class="sd-text-small" x="220" y="114">ESC4 : ACL trop permissive sur un template → reconfiguration en ESC1</text><text class="sd-text-small" x="220" y="132">ESC8 : relais NTLM vers le service web d'inscription HTTP non protégé</text><text class="sd-text-small" x="220" y="150">Certipy : find (énumération) → auth (authentification via le certificat)</text></svg>`,
 def:"Active Directory Certificate Services (AD CS) gère les certificats numériques du domaine. Mal configuré, il devient un des vecteurs de compromission les plus puissants et les plus recherchés en pentest AD moderne.",
 points:["ESC1 : un modèle de certificat permet à l'utilisateur de spécifier son propre Subject Alternative Name (SAN) — il peut demander un certificat au nom d'un administrateur du domaine et s'authentifier à sa place","ESC8 : le service web d'inscription AD CS (HTTP, sans protection contre le relais NTLM) est vulnérable au relais — un attaquant relaie l'authentification d'une machine (ex: via une coercition type PetitPotam) pour obtenir un certificat en son nom","ESC4 : des droits d'écriture trop larges (ACL) sur un modèle de certificat permettent de le reconfigurer pour le rendre exploitable (ex: activer les conditions d'ESC1 dessus)","Certipy et Certify : outils de référence pour énumérer les modèles vulnérables (commande find) et abuser des certificats obtenus pour s'authentifier (commande auth)","Un certificat volé/abusé donne un accès persistant et difficile à révoquer : contrairement à un mot de passe, il reste valide jusqu'à sa date d'expiration ou une révocation explicite du CA","Défense : auditer régulièrement les ACL des modèles de certificats, désactiver l'inscription web HTTP non protégée, et restreindre l'option « Enrollee Supplies Subject » aux templates qui en ont réellement besoin"],
 piege:"AD CS est souvent installé avec des modèles de certificats hérités des paramètres par défaut de Windows Server — beaucoup d'entreprises ne savent même pas qu'ils exposent un chemin d'escalade direct vers Domain Admin, contrairement à Kerberoasting qui est plus connu et donc plus souvent audité.",
 retenir:"AD CS = surface d'attaque via les modèles de certificats. ESC1 = SAN arbitraire, ESC8 = relais NTLM vers le service web d'inscription, ESC4 = ACL trop permissive sur un template. Certipy = outil d'énumération et d'exploitation de référence.",
 keywords:["AD CS","ESC1","ESC8","ESC4","Certipy","Certify","certificat","relais NTLM","modèle de certificat","escalade de privilèges"]}

,{id:5120,cat:"cloud",titre:"Cloud Pentesting — AWS/Azure",sub:"IAM, buckets S3, IMDS/SSRF, escalade de privilèges",
 schema:`<svg viewBox="0 0 440 150" xmlns="http://www.w3.org/2000/svg"><defs><marker id="cp-a" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z" class="sd-arrowhead-red"/></marker></defs><text class="sd-label" x="220" y="12">SSRF → vol des identifiants IAM via IMDS</text><rect class="sd-box" x="6" y="40" width="110" height="40" rx="4"/><text class="sd-text-small" x="61" y="56">Appli vulnérable</text><text class="sd-text-small" x="61" y="70">au SSRF</text><rect class="sd-box-accent" x="164" y="40" width="130" height="40" rx="4" style="stroke:#EF4444"/><text class="sd-text-small" x="229" y="56" style="fill:#EF4444">169.254.169.254</text><text class="sd-text-small" x="229" y="70" style="fill:#EF4444">IMDS (métadonnées)</text><rect class="sd-box" x="340" y="40" width="94" height="40" rx="4"/><text class="sd-text-small" x="387" y="56">Identifiants</text><text class="sd-text-small" x="387" y="70">IAM temporaires</text><line class="sd-arrow" x1="116" y1="60" x2="164" y2="60" marker-end="url(#cp-a)"/><line class="sd-arrow" x1="294" y1="60" x2="340" y2="60" marker-end="url(#cp-a)"/><rect class="sd-box" x="6" y="98" width="428" height="42" rx="4"/><text class="sd-text-small" x="220" y="116">IMDSv2 (jeton requis) atténue ce risque — IMDSv1 reste exploitable si actif</text><text class="sd-text-small" x="220" y="134">Buckets S3 publics + chemins d'escalade IAM = 2 autres vecteurs classiques</text></svg>`,
 def:"Le pentest cloud cible les mauvaises configurations spécifiques aux fournisseurs (IAM trop permissif, stockage exposé, métadonnées accessibles) plutôt que des vulnérabilités logicielles classiques.",
 points:["Énumération IAM : lister les rôles et policies accessibles avec les identifiants obtenus pour repérer une élévation de privilèges possible (ex: combiner la permission de passer un rôle à une instance avec celle de créer une instance)","Bucket S3 mal configuré : un bucket public ou avec des ACL trop larges expose des données sensibles — un test d'accès anonyme suffit souvent à le révéler","IMDS (Instance Metadata Service) et SSRF : une application vulnérable au SSRF peut interroger le service de métadonnées interne pour voler les identifiants temporaires du rôle IAM de l'instance — IMDSv2 (avec jeton) atténue ce risque, IMDSv1 reste exploitable s'il est encore actif","Azure : les Managed Identities et le endpoint de métadonnées équivalent suivent la même logique de risque SSRF que sur AWS","Chemins d'escalade de privilèges IAM : des combinaisons de permissions en apparence anodines (ex: pouvoir modifier une policy IAM soi-même) peuvent mener à un accès administrateur complet — des outils cartographient ces chemins automatiquement","Clés d'accès codées en dur : rechercher des clés AWS/Azure fuitées dans du code source public, des variables d'environnement exposées, ou des images de conteneurs mal nettoyées"],
 piege:"Beaucoup considèrent le fournisseur cloud responsable de toute la sécurité — le modèle de responsabilité partagée signifie que la configuration IAM, les buckets et les identifiants restent entièrement de la responsabilité du client, pas du fournisseur.",
 retenir:"Pentest cloud = configuration, pas exploitation logicielle. IMDS/SSRF = vol d'identifiants temporaires. Buckets publics = fuite de données classique. Chemins d'escalade IAM = combinaisons de permissions à cartographier.",
 keywords:["cloud pentesting","AWS","Azure","IAM","S3","IMDS","SSRF","escalade de privilèges","métadonnées","responsabilité partagée"]}

,{id:5121,cat:"devops",titre:"Supply Chain Security",sub:"Dependency confusion, typosquatting, SBOM, CI/CD",
 schema:`<svg viewBox="0 0 440 160" xmlns="http://www.w3.org/2000/svg"><defs><marker id="sc-a" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z" class="sd-arrowhead-red"/></marker></defs><text class="sd-label" x="220" y="12">Points d'entrée de la chaîne d'approvisionnement</text><rect class="sd-box" x="6" y="30" width="120" height="28" rx="3"/><text class="sd-text-small" x="66" y="48">Dépendance piégée</text><rect class="sd-box" x="6" y="62" width="120" height="28" rx="3"/><text class="sd-text-small" x="66" y="80">Compte mainteneur</text><rect class="sd-box" x="6" y="94" width="120" height="28" rx="3"/><text class="sd-text-small" x="66" y="112">Pipeline CI/CD</text><rect class="sd-box-accent" x="164" y="60" width="110" height="32" rx="4" style="stroke:#EF4444"/><text class="sd-text-small" x="219" y="80" style="fill:#EF4444">Artefact final</text><line class="sd-arrow" x1="126" y1="44" x2="164" y2="70" marker-end="url(#sc-a)"/><line class="sd-arrow" x1="126" y1="76" x2="164" y2="76" marker-end="url(#sc-a)"/><line class="sd-arrow" x1="126" y1="108" x2="164" y2="82" marker-end="url(#sc-a)"/><line class="sd-arrow" x1="274" y1="76" x2="330" y2="76" marker-end="url(#sc-a)"/><rect class="sd-box" x="332" y="60" width="100" height="32" rx="4"/><text class="sd-text-small" x="382" y="80">Utilisateurs</text><rect class="sd-box" x="6" y="130" width="426" height="24" rx="4"/><text class="sd-text-small" x="220" y="146">SBOM = inventaire pour détecter/réagir vite quand une dépendance est compromise</text></svg>`,
 def:"La sécurité de la chaîne d'approvisionnement logicielle (supply chain) protège contre la compromission indirecte via des dépendances, des outils de build ou des fournisseurs tiers, plutôt que l'application elle-même.",
 points:["Dependency confusion : publier un package public portant le même nom qu'un package interne privé — si la configuration de résolution privilégie le registre public, le code malveillant est installé à la place du package légitime","Typosquatting : publier un package au nom très proche d'un package populaire, en comptant sur une faute de frappe des développeurs lors de l'installation","Compromission de mainteneur ou de compte : un attaquant qui prend le contrôle du compte d'un mainteneur légitime peut publier une version piégée d'un package massivement utilisé","SBOM (Software Bill of Materials) : inventaire complet des composants et dépendances d'un logiciel — permet de réagir rapidement à une vulnérabilité découverte dans une dépendance largement utilisée","Compromission de pipeline CI/CD : un attaquant qui accède au pipeline de build peut injecter du code malveillant directement dans l'artefact final, sans jamais toucher au dépôt source","Bonnes pratiques : verrouiller les versions de dépendances (lockfiles), signer les artefacts de build, scanner automatiquement les dépendances et images (Dependabot, Snyk, Trivy)"],
 piege:"Auditer uniquement son propre code ne suffit pas : la majorité des vulnérabilités d'une application moderne proviennent de ses dépendances tierces, pas du code écrit en interne.",
 retenir:"Supply chain = compromission indirecte via dépendances, CI/CD ou fournisseurs. Dependency confusion et typosquatting ciblent les développeurs. SBOM = inventaire pour réagir vite. Verrouiller les versions + scanner = défenses de base.",
 keywords:["supply chain","dependency confusion","typosquatting","SBOM","CI/CD","Dependabot","Snyk","Trivy","chaîne d'approvisionnement"]}
